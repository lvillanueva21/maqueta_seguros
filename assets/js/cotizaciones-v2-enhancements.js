(() => {
  ...
  function nextQuoteCode(records) {
    ...
    const values = records.filter((item) => String(item.code || '').startsWith(prefix)).map((item) => Number(String(item.code).slice(prefix.length))).filter(Number.isFinite);
  ...
  function duplicateQuote(id) {
    ...
    (clone.groups || []).forEach((group) => (group.alternatives || []).forEach((alt) => { if (alt.state === 'Elegida') alt.state = 'Disponible'; }));
  ...
  function duplicateTemplate(id) {
    ...
    note('success', 'La plantilla fue duplicada para que puedas ajustarla sin alterar la original.', { title: 'Plantilla duplicada' });
    location.reload();
  }
  function decorate() {
    addContext();
    $$('.quote-list-card').forEach((card) => {
      ...
      if (quoteRequested && String(opener.dataset.open) === String(quoteRequested)) window.setTimeout(() => opener.click(), 0);
    });
  ...
  new MutationObserver(() => window.setTimeout(decorate, 0)).observe(document.body, { childList: true, subtree: true });
  window.setTimeout(() => {
    decorate();
    if (action === 'nueva') $('#quote-new')?.click();
  }, 100);
})();